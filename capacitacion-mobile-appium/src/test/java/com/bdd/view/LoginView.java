...
public class LoginView extends MobileBase {
    ...
    private MobileElement msgErrorUserName;
    ...
    public void seleccionarOpcionMenu(String sMenu) {
        ...
        Util.takeScreenShoot();
        String sXpath = "//android.view.ViewGroup[@content-desc='menu item log in']";
        MobileElement option = getMobileElementByXPath(sXpath);
        clickElement(option);
    }
    ...
    public boolean validarAlerta(String mensajeEsperado) {
        explicitWaiting(3000, driver -> ExpectedConditions.visibilityOf(msgErrorUserName));
        Util.logger(this.getClass()).log(Level.WARNING, "validando si existe mensaje de error: " + mensajeEsperado);
        Util.takeScreenShoot();
        return getText(msgErrorUserName).equals(mensajeEsperado);
    }
}
